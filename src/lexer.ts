...
export class Lexer {
  ...
  private tokenizeIdentifierOrKeyword(): Token {
    const start = this.position - 1;
    while (
      this.isLetter(this.peekNextChar()) ||
      this.isDigit(this.peekNextChar())
    ) {
      this.getNextChar();
    }
  ...
  private tokenizeOperatorOrPunctuation(): Token | null {
    ...
    throw new Error(
      `Unexpected character: ${char} at position ${this.position} in input: ${this.input} with next char: ${nextChar}`
    );
  }
