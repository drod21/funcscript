...
class Parser {
    ...
    private parseIfStatement(): ASTNode {
        ...
        return new ASTNode(ASTNodeType.IfStatement, null, alternate ? [condition, consequent, alternate] : [condition, consequent]);
    }
    ...
    private parseMatchCase(): ASTNode {
        ...
        const expression = this.parseExpression();
        return new ASTNode(ASTNodeType.MatchCase, pattern.value, [expression]);
    }
    ...
    private parseFunctionDeclaration(): ASTNode {
        ...
        return new ASTNode(ASTNodeType.FunctionDeclaration, identifier.value, [parameters, body]);
    }
    ...
    private parseParameters(): ASTNode {
        ...
        return new ASTNode(ASTNodeType.Parameters, null, parameters.map(param => new ASTNode(ASTNodeType.Identifier, param)));
    }
    ...
    private parseIfExpression(): ASTNode {
        if (this.currentToken().type === TokenType.Keyword && this.currentToken().value === "if") {
            ...
            return new ASTNode(ASTNodeType.IfExpression, null, alternate ? [condition, consequent, alternate] : [condition, consequent]);
        }
